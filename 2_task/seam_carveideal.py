...
def seam_carve(imag, mode, mask = None):
    ...
    energy[1 : lenn - 1, 1 : wid - 1] = np.sqrt((bright[2 :, 1 : wid - 1] - bright[0 : lenn - 2, 1 : wid - 1])**2 + (bright[1 : lenn -1, 2 :] - bright[1: lenn - 1, 0 : wid - 2])**2)
    energy[0,0] = sqrt((bright[0,1] - bright[0,0])**2 + (bright[1,0] - bright[0,0])**2)
    ...
    return retim, retmask, retseamask  #,energy, bright, seams, seamcoord
def convert_img_to_mask(img):
    return ((img[:, :, 0] != 0) * -1 + (img[:, :, 1] != 0)).astype('int8')

img = imread("task_tests\\01_input\img.png")
mask = convert_img_to_mask(imread('task_tests\\01_input\mask.png'))
im,ma = seam_carve(img, 'vertical expand', None)[0:2]
for i in range(30):
    im,ma = seam_carve(im, 'vertical expand', ma)[0:2]
imshow(im)
